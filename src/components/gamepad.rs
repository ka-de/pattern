...
const BUTTON_RADIUS: f32 = 25.0;
const BUTTON_CLUSTER_RADIUS: f32 = 50.0;
const START_SIZE: Vec2 = Vec2::new(30.0, 15.0);
const TRIGGER_SIZE: Vec2 = Vec2::new(70.0, 20.0);
const STICK_BOUNDS_SIZE: f32 = 100.0;

const BUTTONS_X: f32 = 150.0;
const BUTTONS_Y: f32 = 80.0;
const STICKS_X: f32 = 150.0;
const STICKS_Y: f32 = -135.0;
...
fn setup(mut commands: Commands, meshes: Res<ButtonMeshes>, materials: Res<ButtonMaterials>) {
    ...
    commands
        .spawn(SpatialBundle {
            transform: Transform::from_xyz(BUTTONS_X, BUTTONS_Y, 0.0),
            ..default()
        })
        ...
        commands
            ...
            .with_children(|parent| {
                ...
                parent.spawn(SpriteBundle {
                    sprite: Sprite {
                        custom_size: Some(Vec2::splat(STICK_BOUNDS_SIZE * 2.0)),
                        color: DEAD_COLOR,
                        ..default()
                    },
                    ..default()
                ...
                // live zone
                parent.spawn(SpriteBundle {
                    transform: Transform::from_xyz(live_mid, live_mid, 2.0),
                    sprite: Sprite {
                ...
                // dead zone
                parent.spawn(SpriteBundle {
                    transform: Transform::from_xyz(dead_mid, dead_mid, 3.0),
                    sprite: Sprite {
                ...
                parent.spawn((
                    Text2dBundle {
                        transform: Transform::from_xyz(0.0, STICK_BOUNDS_SIZE + 2.0, 4.0),
                ...
                parent.spawn((
                    MaterialMesh2dBundle {
                        ...
                        transform: Transform::from_xyz(0.0, 0.0, 5.0).with_scale(
                            Vec2::splat(0.15).extend(1.0)
                        ),
                        ..default()
                    },
                    MoveWithAxes {
...
fn setup_triggers(
            ...
            .with_children(|parent| {
                parent.spawn((
                    Text2dBundle {
                        ...
                        text: Text::from_section(format!("{:.3}", 0.0), TextStyle {
                            font_size: 16.0,
                            ..default()
                        }),
                        ..default()
    ...
    spawn_trigger(-BUTTONS_X, BUTTONS_Y + 145.0, GamepadButtonType::LeftTrigger2);
    spawn_trigger(BUTTONS_X, BUTTONS_Y + 145.0, GamepadButtonType::RightTrigger2);
}
...
fn update_connected(
    ...
    text.sections[1].value = if !formatted.is_empty() { formatted } else { "None".to_string() };
}
